...
def plot_scatter_compare_race_incident_vs_population_pulled(
    df,
    df_reference,
    n_county=5,
    annotate=False,
    ...
    topN = df['incident_county'].value_counts()[:n_county].index
    gb = df.loc[df['incident_county'].isin(topN)].groupby(['year', 'civilian_race'])
    topN_race_count = gb['date_incident'].count().unstack().fillna(0).sum()
    topN_race_ratio = topN_race_count/topN_race_count.sum()

    topN_pop = df_reference.loc[topN].sum()
    topN_pop_ratio = topN_pop/topN_pop.sum()
    topN_pop_ratio = topN_pop_ratio[race_list]

    df_merged = pd.concat([topN_pop_ratio, topN_race_ratio], axis=1)
    df_merged.columns = ['General Population', 'OIS Incidents']
    df_merged['Ratio'] = df_merged['OIS Incidents']/df_merged['General Population']

    df_merged.plot.scatter('General Population', 'OIS Incidents', marker='o', 
                        s=75, color=cols_race, ax=ax)
    for race, val in zip(race_list, df_merged.values):
        if annotate:
            ax.annotate('{}\n{:.1f}x'.format(
                race[0],
                df_merged.loc[race, 'Ratio']), 
                (val[0]+0.03, val[1]), fontsize=10)
        else:
            ax.annotate(race[0], (val[0]+0.03, val[1]), fontsize=10)
    ax.plot([0, 1], [0, 1], ':', color='k', alpha=0.5)
    ax.set(ylabel='OIS Data', xlabel='General Population', 
        xticks=np.linspace(0, 1, 6), yticks=np.linspace(0, 1, 6), 
        xlim=[0, xylim], ylim=[0, xylim])
